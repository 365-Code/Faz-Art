...
import { addCategory, fetchCategories, addProduct } from "@/lib/actions";
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage your marble collections and products
            </p>
          </div>
...
const AddCategory = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const uploadedImage: ProductImage | null = await uploadToCloudinary(
        categoryImageFile
      );
      ...
      formData.append("image", JSON.stringify(uploadedImage)); // Stringify the image object
      formData.append("description", categoryDescription);

      await addCategory(formData);
      toast.success("Category added successfully!");
      setIsAddCategoryOpen(false); // Close dialog on success
    ...
    <Dialog open={isAddCategoryOpen} onOpenChange={setIsAddCategoryOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new marble category for your collection.
          </DialogDescription>
        </DialogHeader>
...
const AddProduct = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setIsSubmitting(true); // Start loading
    try {
      const uploadedImages: ProductImage[] = await uploadMultipleToCloudinary(
        productImageFiles
      );
      ...
      uploadedImages.forEach((img) => {
        formData.append("images", JSON.stringify(img)); // Stringify each image object
      });

      await addProduct(formData);
      toast.success("Product added successfully!");
      setIsAddProductOpen(false); // Close dialog on success
    ...
    <Dialog open={isAddProductOpen} onOpenChange={setIsAddProductOpen}>
      ...
      <DialogContent>
        ...
        <form
          ...
          className="space-y-4 overflow-x-hidden custom-scrollbar max-h-[calc(100vh-150px)] overflow-y-auto"
        >
          <div className="space-y-2">
